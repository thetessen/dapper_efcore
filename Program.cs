...
namespace DapperEFCorePerformanceBenchmarks
{
    class Program
    {
        ...
        public static List<TestResult> RunTests(int runID, Framework framework, ITestSignature testSignature)
        {
            ...
            result.PlayerByIDMilliseconds = Math.Round(playerByIDResults.Average(), 5);
            ...
            */
            ...
            createResults.Add(testSignature.Create());
            result.CreateMilliseconds = Math.Round(createResults.Average(), 5);

            List<long> updateResults = new List<long>();
            updateResults.Add(testSignature.Update());
            result.UpdateMilliseconds = Math.Round(updateResults.Average(), 5);

            List<long> deleteResults = new List<long>();
            deleteResults.Add(testSignature.Delete());
            result.DeleteMilliseconds = Math.Round(deleteResults.Average(), 5);
            results.Add(result);
        ...
        public static void ProcessResults(List<TestResult> results)
        {
            ...
            foreach (var group in groupedResults)
            {
                ...
                Console.WriteLine("Run #\t\tSlect\t\t\tCeate\t\t\tUpdate\t\t\tDelete");
                ...
                foreach (var orderResult in orderedResults)
                {
                    ...
                    Console.WriteLine((orderResult.Run).ToString() + "\t\t" + orderResult.PlayerByIDMilliseconds + "\t\t\t" + orderResult.CreateMilliseconds + "\t\t\t" + orderResult.UpdateMilliseconds + "\t\t\t" + orderResult.DeleteMilliseconds);
