...
namespace DapperEFCorePerformanceBenchmarks.DataAccess
{
    public class Dapper : ITestSignature
    {
        ...
        public long Create()
        {
            ...
            {
                conn.Open();
                conn.Query<SportDTO>("INSERT INTO sport(Id,Name) VALUES(@Id,@Name )", new { ID = 12 ,Name = "Esport"});
            }
            watch.Stop();
            return watch.ElapsedMilliseconds;
        }
        public long Update()
        {
            ...
            {
                conn.Open();
                conn.Query<SportDTO>("Update sport SET Name = @Name Where Id = @ID", new { ID = 12, Name = "Esport" });
            }
            watch.Stop();
            return watch.ElapsedMilliseconds;
        }
        public long Delete()
        {
            ...
            {
                conn.Open();
                conn.Query<SportDTO>("Delete sport Where Id = @ID", new { ID = 12 });
            }
            watch.Stop();
            return watch.ElapsedMilliseconds;
        }
